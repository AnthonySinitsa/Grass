Shader "Unlit/BillboardGrass"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 RotateAroundYInDegrees(float3 vertex, float4 rotation) {
                float alpha = rotation.y * UNITY_PI / 180.0;
                float sina, cosa;
                sincos(alpha, sina, cosa);
                float2x2 m = float2x2(cosa, -sina, sina, cosa);
                return float3(mul(m, vertex.xz), vertex.y);
            }
            ...
            v2f vert(appdata_full v, uint id : SV_InstanceID)
            {
                ...
                float3 localPosition = RotateAroundYInDegrees(v.vertex, grassRotation);
